...
<main class="mx-auto max-w-2xl px-4 py-8 space-y-6">
	...
	<!-- Profile facts -->
	{#if p.business_type || p.location || p.started_year || p.employee_count || p.monthly_revenue_estimate || p.products_or_services || (p.sales_channels?.length)}
		<SectionCard title="Identitas usaha">
			<dl class="space-y-3 text-sm">
				{#if p.business_type}<div class="flex justify-between"><dt class="text-slate-500">Jenis usaha</dt><dd class="font-medium text-slate-950">{p.business_type}</dd></div>{/if}
				{#if p.location}<div class="flex justify-between"><dt class="text-slate-500">Lokasi</dt><dd class="font-medium text-slate-950">{p.location}</dd></div>{/if}
				{#if p.started_year}<div class="flex justify-between"><dt class="text-slate-500">Tahun mulai</dt><dd class="font-medium text-slate-950">{p.started_year}</dd></div>{/if}
				{#if p.employee_count}<div class="flex justify-between"><dt class="text-slate-500">Karyawan</dt><dd class="font-medium text-slate-950">{p.employee_count} orang</dd></div>{/if}
				{#if p.monthly_revenue_estimate}<div class="flex justify-between"><dt class="text-slate-500">Omzet bulanan</dt><dd class="font-medium text-slate-950">{p.monthly_revenue_estimate}</dd></div>{/if}
				{#if p.products_or_services}<div class="flex justify-between"><dt class="text-slate-500">Produk/layanan</dt><dd class="font-medium text-slate-950">{p.products_or_services}</dd></div>{/if}
			</dl>
	...
	{#if p.business_needs || p.growth_target || p.main_challenges}
		<SectionCard title="Kebutuhan & target">
			<dl class="space-y-3 text-sm">
				{#if p.business_needs}<div class="flex justify-between"><dt class="text-slate-500">Kebutuhan</dt><dd class="font-medium text-slate-950">{p.business_needs}</dd></div>{/if}
				{#if p.growth_target}<div class="flex justify-between"><dt class="text-slate-500">Target</dt><dd class="font-medium text-slate-950">{p.growth_target}</dd></div>{/if}
				{#if p.main_challenges}<div class="flex justify-between"><dt class="text-slate-500">Kendala</dt><dd class="font-medium text-slate-950">{p.main_challenges}</dd></div>{/if}
			</dl>
		</SectionCard>
	{/if}
			...
			<div class="space-y-2 text-sm">
				{#if p.strengths?.length}
					{#each p.strengths as strength (strength)}
						<div class="flex items-center gap-2 text-slate-900"><span class="text-emerald-500 shrink-0">✓</span>{strength}</div>
					{/each}
				{/if}
				{#if p.risks?.length}
					{#each p.risks as risk (risk)}
						<div class="flex items-center gap-2 text-slate-700"><span class="text-amber-500 shrink-0">!</span>{risk}</div>
					{/each}
				{/if}
				{#if p.evidence_summary}
					<p class="mt-1 text-slate-500">Bukti aktivitas: {p.evidence_summary}</p>
				{/if}
			</div>
		</SectionCard>
	{/if}
